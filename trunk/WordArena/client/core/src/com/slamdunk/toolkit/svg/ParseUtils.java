...
public class ParseUtils implements SVGConstants{
	...
	public static float extractNumberAttribute(String val) {
		if ( val != null ) {
			try {
				if ( val.endsWith(UNIT_PX) ) {
					return Float.parseFloat(val.substring(0, val.length() - 2));
				} else {
	...
	public static float[] extractPoints(String property) {
		String[] strPoints = property.trim().split(" ");
		int pointsLenght   = strPoints.length;	
		float[] points    = new float[pointsLenght*2];
			...
			if ( isCharacter(command) ){
				...
				if ( !"z".equals(command) && !"Z".equals(command) ){
					while( x < (length-1) && !isCharacter(temp[++x]) ){
						values.add(Float.parseFloat(temp[x]));
					}
					x--;
					
					retorno.add(new SVGPath(command.charAt(0), vectorToFloatArray(values)));
				}
	...
	private static float[] vectorToFloatArray(Vector<Float> vector){
	...
	public static SVGStyle[] extractStyleProperty(String style) {
		...
		for ( int x=0; x < length; x++ ){
			...
			if ( ATTRIBUTE_FILL.equals(properties[0])  || 
				...
				retorno[x] = new SVGStyle(properties[0], properties[1], 0);
